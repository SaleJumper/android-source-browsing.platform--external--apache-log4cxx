        ...
        helpers::Properties& props, const LogString& appenderName)
{
        ...
        Pool p;
                ...
                if ( rfa != NULL )
                {
                        ...
                        // most RFAs will have a rollingPolicy, but it's not strictly mandatory as
                        // the RFA falls back to a FixedWindowRollingPolicy
                        LogString rollingPolicyPrefix = prefix + LOG4CXX_STR(".rollingPolicy");
                        LogString rollingPolicyName(props.getProperty(rollingPolicyPrefix));
                        if (!rollingPolicyName.empty())
                        {
                                // ***CS***
                        ...
                        // RFAs can have a triggeringPolicy as well. If there is none, the RFA will
                        // use the rollingPolicy (if it's a triggeringPolicy as well) or
                        // ManualTriggeringPolicy
                        LogString triggeringPolicyPrefix = prefix + LOG4CXX_STR(".triggeringPolicy");
                        LogString triggeringPolicyName(props.getProperty(triggeringPolicyPrefix));
                        if (!triggeringPolicyName.empty())
                        {
